...
その直下に企業一括ダウンロードを `st.columns(2)` で配置（企業未選択時は disabled、§6.2）。
...
### 6.2 企業一括ダウンロード（上部コントロール直下の2ボタン）

選択中の企業名称＋半径を対象に、上部コントロール直下へ `st.columns(2)` で2ボタンを配置する
（企業未選択時は `disabled`）。

- **企業一括データダウンロード**: 対象企業＋半径で絞り込んだデータを **1つのCSV**で出力する
  （店舗ごとの分割・ZIP は行わない）。
  ...
  - CSV は軽量のため `st.download_button` で直接生成。
- **企業一括画像ダウンロード**: 対象企業の全店舗について §6.1.2/§8.3 と同一描画ロジックで
  PNG を生成し ZIP にまとめる。生成が重いため `st.button("企業一括画像を生成")` →
  `st.progress` 付き生成 → `st.session_state` 保持 → `st.download_button("画像ZIPをダウンロード")`
  の2段とする。
  - 1ファイル名: `{小売店名称}.png` / ZIPファイル名: `{企業名称}_{半径}km.zip`
