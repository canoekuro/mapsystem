...
- folium.Map 仕様
  ...
  - 情報粒度（詳細度）の固定: `config/theme.toml` `[map] map_detail_zoom`（既定 `0`）・テーマ設定ページで
    変更可。`0` は固定しない（従来どおりズームに追従）。`1–19` のときは `TileLayer` の
    `max_native_zoom`/`min_native_zoom` を同値（選択中ベースマップの `max_zoom` へクランプ）に設定し、
    地図をズームしてもタイルを拡大縮小するだけにして表示情報の粒度を一定に保つ。対話地図のみ（PNGは対象外）。
    `lib/colors.map_detail_zoom()` で描画時に解決する。
