...
| カラム名（アプリ内部名） | 型 | 必須 | 説明 |
|---|---|---|---|
| 企業G名称 | str | - | 企業グループ名称（issue 202607231113 で追加。現状は表示未使用） |
| 企業名称 | str | ○ | 例: ヨークベニマル |
...
- **ローデータダウンロード** / **店舗別推進園数ダウンロード**: 取得済み企業全体
  （`企業名称==指定企業 かつ 距離<=半径`）が対象。CSV は **cp932**
  （`to_csv(index=False).encode("cp932", errors="replace")`）。ファイル名は
  `{企業名称}_{半径}km.csv` / `{企業名称}_{半径}km_店舗別推進園数.csv`。
- **商談用資料ダウンロード** / **店舗POPダウンロード**: 選択中の 1 店舗が対象。地図画像
  （地図のみ PNG）を PowerPoint テンプレートの画像プレースホルダー位置へ**アスペクト比を保ったまま**
  貼り付けた pptx を `@st.cache_data` で生成する（§8.5）。ファイル名は
  `{小売店名称}_商談用資料.pptx` / `{小売店名称}_店舗POP.pptx`。店舗未選択／圏内推進園0件では `disabled`。
...
| F-04 | 店舗単位 商談用資料/店舗POP ダウンロード | 選択中の小売店名称、半径(km) | `{小売店名称}_商談用資料.pptx` / `{小売店名称}_店舗POP.pptx`（地図画像を貼付した1スライド） |
...
企業単位の店舗別 PNG 一括 ZIP は廃止した（issue 202607221128、§8.5 の pptx 出力に置換）。
`lib/png_builder.py` / `lib/zip_builder.py` の合成 PNG・ZIP 生成ロジックは当面残置するが、
画面からは参照しない。
...
│  ├ upload_page.py       # データ更新（Excel .xlsx/.xls を Volume へ格納、拡張子は保持）
...
│  └ zip_builder.py       # 画像ZIP 生成（現在は未使用・残置）
├ config/
