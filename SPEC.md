...
- **取得方式:** オンデマンド取得。起動時は全件をロードせず、企業名称の DISTINCT のみを軽量クエリで取得する
  （候補は小売店マスタ `store_table` の `企業名称` から取得する。issue 202607221245）。
  ユーザーが企業名称と取得半径を指定して「データ取得」を押下したとき、`企業名称 == 指定企業 AND 距離km <= 取得半径`
  を Spark 側で適用してから `toPandas()` で取得する（大量データの転送を回避）。

- **小売店マスタテーブル:** `[databricks] store_table` で指定する別テーブル（`企業名称/店舗名称/店舗コード/店舗_都道府県`
  の DISTINCT）。「データ取得」時に、上記の店舗×推進園テーブル（距離で絞る）とは別に、`企業名称 == 指定企業`
  のみで（距離条件なしで）取得する。取得半径圏内に推進園が 0 件の店舗も残すためのマスタで、小売店名称・都道府県の
  **選択肢**と**店舗別推進園数の集計表の土台**に用いる（issue 202607221128）。
  ```sql
  CREATE OR REPLACE TABLE <store_table> AS
  SELECT DISTINCT 企業名称, 店舗名称, 店舗コード, 店舗_都道府県 FROM <table>
  ```
...
アプリ内部名は `config/column_mapping.toml` の左辺で固定し、実テーブル列名（右辺）を張り替えて
追随する。実テーブル側では `店舗コード` が **`小売店CD`** へ改名され、`企業G名称` と出荷実績9列が
追加された（issue 202607231113）。アプリ内部名は従来どおり `店舗コード` を用いる。

| カラム名（アプリ内部名） | 型 | 必須 | 説明 |
|---|---|---|---|
| 企業G名称 | str | - | 企業グループ名称（issue 202607231113 で追加）。本部担当用ページの取得キー・表示に使用（§7 F-09, issue 202607231301） |
| 企業名称 | str | ○ | 例: ヨークベニマル |
| 業態名称 | str | - | 例: GMS（新スキーマの列一覧からは外れているが未使用。mapping は残置） |
| 店舗コード | str | ○ | 店舗一意キー。実テーブル列は `小売店CD`（mapping で張替） |
| 店舗名称 | str | ○ | 例: アピタ千代田橋店 |
| 店舗_都道府県 | str | ○ | 店舗の都道府県（画像一括DLの絞込に使用） |
| 店舗住所 | str | ○ | 表示用 |
| 店舗lat | float | ○ | WGS84 |
| 店舗lon | float | ○ | WGS84 |
| {商材}_当年実績（箱数） | float | - | 出荷実績。商材は プラズマ計 / おい免 / ムテキッズ。マップの出荷実績表・店舗別表で表示（§6.1.2） |
| {商材}_前年実績（箱数） | float | - | 出荷実績（前年）。店舗別 推進園数表で表示 |
| {商材}_前年比 | float | - | 前年比（**比率**。例 1.053）。表示側で ×100 して `%`（小数1桁）に整形 |
| 推進園名称 | str | ○ | 推進園名 |
...
| 推進園lat | float | ○ | WGS84 |
| 推進園lon | float | ○ | WGS84 |
| 距離km | float | ○ | km、事前算出済み。値はフル精度で渡るため、表示時に小数第1位へ丸める（issue 202607231301） |

> **出荷実績の対象期間:** データ更新ジョブが Volume（`[rdp] update_toml_path`、既定
> `/Volumes/kh_aigrd_databricks_prd_01/default/immune_promo_nursery/rdp_update.toml`）へ
> `プラズマドライ計 = 期間①（YYYY年MM月 ～ YYYY年MM月）` 形式で書き出す。マップの出荷実績表の
> 「出荷実績　期間：」表記に用いる（プラズマの日付のみ使用。`lib.data.load_shipment_period`）。
...
- 地図は**固定画面**とする。拡大縮小・移動（パン/ズーム）を一切無効化し、`build_map` の初期中心/
  ズームで固定表示する。`folium.Map` で `zoom_control` / `dragging` / `scrollWheelZoom` /
  `doubleClickZoom` / `touchZoom` / `boxZoom` / `keyboard` をすべて `False` に設定する。
  誤操作でズレないため、旧「マップをリセット」ボタン（`st_folium` の `key` に nonce を含め再マウント
  する方式）は廃止した。
- folium.Map 仕様
  - `location=[店舗緯度, 店舗経度]`
  ...
  - `tiles`/`attr`/`max_zoom`: 選択中のベースマップ（`lib/basemaps.py` の catalog、`config/theme.toml`
    `[map] basemap` で指定・テーマ設定ページで変更可）から取得する。`folium.Map(tiles=None)` の上に
    明示的な `folium.TileLayer` として重ね、帰属表示（`attr`）を地図に表示する。
    背景の選択肢: **OSM**（標準 / Humanitarian / 日本語スタイル(OSMFJ)）・**国土地理院**（標準 / 淡色 /
    航空写真 / 白地図）・**CARTO(OSMベース)**（Positron / Voyager / Dark Matter）。いずれもラスターXYZタイル。
  - 情報粒度（詳細度）の固定: `config/theme.toml` `[map] map_detail_zoom`（既定 `0`）・テーマ設定ページで
    変更可。`0` は固定しない（ズームに追従）が、**情報粒度（タイル取得ズーム）が 15 以上になるときは
    14 で頭打ち**にする（`TileLayer` の `max_native_zoom` を 14 に設定。14 以下はズームに追従）。
    `1–14` のときは `max_native_zoom`/`min_native_zoom` を同値（選択中ベースマップの `max_zoom` へ
    クランプ）に設定し、地図をズームしてもタイルを拡大縮小するだけにして表示情報の粒度を一定に保つ。
    ダウンロードPNG（`lib/static_map.py`）でも同様に、表示ズームは維持しつつ取得ズームを 14 で頭打ちにして
    14 相当タイルを拡大して用いる。`lib/colors.map_detail_zoom()` で描画時に解決する。
...
- 店舗マーカー
  - `folium.Marker`
  - 中心の店舗のみ表示
  - アイコン: 同梱画像 `images/icon.png`（買い物カート入りのピン型）を `folium.CustomIcon` で表示する。
    サイズはテーマの相対サイズ（`store_marker_size`）で調整し、ピン先端（tip）を店舗座標へ合わせる
    （`icon_anchor`）。アイコン画像・先端位置の解決は `lib/icons.py` に一元化し、対話地図・PNGで共有する。
  - tooltip: 小売店名称
- 推進園マーカー（距離 ≤ 指定半径）
  ...
  - tooltip: `{連番}. {施設名称}（{距離:.1f}km）`（距離は小数第1位まで, issue 202607231301）
  - サイズ（相対％）は**地図半径ごとに段階指定**する。`config/theme.toml` `[map]`
    `facility_marker_sizes`（既定 `[150, 120, 100, 90, 80, 50]`、半径バケット
    `[≤1, ≤2, ≤3, ≤4, ≤5, >5]` km に対応）・テーマ設定ページで変更可。`lib/colors.py`
    `facility_marker_size_for_radius(radius_km)` で描画時に解決し、対話地図・PNG両方に反映する。
...
- カードレイアウト（HTML+CSS）:
  ...
  - 右側に施設名称（太字 14px）と距離（淡色 12px、フォーマット `約{距離:.1f}km`、小数第1位まで, issue 202607231301）
  - 背景: `#FFFFFF`
...
##### 出荷実績テーブル（施設リスト下, issue 202607231113）

施設リストの下に、選択中1店舗の出荷実績を image1.png 準拠の 2 値列で表示する
（`views/main_page.py:_sales_table_html`）。

- 行: 商材（`プラズマ計` / `おい免` / `ムテキッズ`、`lib.data.SALES_PRODUCTS`）
- 列: **実績（箱数）**＝`{商材}_当年実績（箱数）`（DB値をそのまま表示）、
  **前年比（%）**＝`{商材}_前年比`（DB は比率のため ×100 して小数1桁＋`%`）。欠損は `—`
- 値の出所は `load_filtered` の選択店舗行（`srow`）
- テーブル右下に `出荷実績　期間：{period}`。`period` は `lib.data.load_shipment_period()`
  （`rdp_update.toml` を解析。取得不可時は `—`）

##### 件数表示

ヘッダー直下に `st.metric("対象推進園数", f"{n}件")` を配置。

##### 件数表示のみ（ダウンロードは sidebar）

商談用資料・店舗POP を含むダウンロード系ボタンは sidebar に集約した（§6.2、issue 202607221245）。
地図（`st_folium`）の直下では pptx 用の地図画像（`lib.static_map.render_static_map`）を
対話地図と同寸（`map_width×map_height`）で 1 回だけ生成し、sidebar のボタンで使い回す。

### 6.2 ダウンロード（sidebar に集約）

ファイルダウンロード系のボタンは、ページナビが並ぶ **sidebar**（`st.sidebar`）へまとめて配置する
（issue 202607221245、旧「データダウンロード」`st.expander` は廃止）。データ取得後に表示し、
以下 2 ボタンを縦に並べる（ローデータダウンロードは廃止, issue 202607221414）。

- **店舗別推進園数ダウンロード**: 取得済み企業全体
  （`企業名称==指定企業 かつ 距離<=半径`）が対象。CSV は **cp932**
  （`to_csv(index=False).encode("cp932", errors="replace")`）。ファイル名は
  `{企業名称}_{半径}km_店舗別推進園数.csv`。
- **商談資料・店舗POPダウンロード**: 選択中の 1 店舗が対象。実運用では商談用資料と店舗POPを
  両方DLするケースが大半のため 1 ボタンに統合し、押下で両 pptx を 1 つの ZIP にまとめて
  ダウンロードする（issue 202607231301、`lib.zip_builder.build_pptx_zip`）。各 pptx は地図画像
  （地図のみ PNG）を PowerPoint テンプレートの画像プレースホルダー位置へ**アスペクト比を保ったまま**
  貼り付けて `@st.cache_data` で生成する（§8.5）。ZIP 内ファイル名は
  `{小売店名称}_商談用資料.pptx` / `{小売店名称}_店舗POP.pptx`、ダウンロード名は
  `{小売店名称}_商談資料・店舗POP.zip`（mime `application/zip`）。店舗未選択／圏内推進園0件では `disabled`。
  テンプレは `[pptx] template_dir` の Volume から取得し、失敗時は `images/template.pptx`。
...
- 調整対象（配色）: 推進園区分の色（凡例・マーカー）、区分フォールバック色、半径円の線色、
  半径円の塗り透明度（`st.slider` 0.0–0.5）、見出し帯・施設リストヘッダー色。
...
- 調整対象（マーカーサイズ）: 推進園マーカーは**地図半径別に6段階**（`[≤1,≤2,≤3,≤4,≤5,>5]`km、
  各 `st.number_input` 50–200％）、店舗マーカーは単一の相対％（`st.number_input` 50–200）。
  `config/theme.toml` `[map] facility_marker_sizes`/`store_marker_size` に保存し、地図・PNG両方へ反映する。
- プレビュー: 選択中の配色で見出し帯・凡例・施設リストバッジ・半径円を HTML で表示。
...
| ID | 機能 | 入力 | 出力 |
|---|---|---|---|
| F-01 | 店舗単位マップ表示（店舗担当用ページ） | 企業名称→小売店名称、半径(km) | 地図、件数、施設リスト |
| F-04 | 店舗単位 商談資料・店舗POP ダウンロード | 選択中の小売店名称、半径(km) | `{小売店名称}_商談資料・店舗POP.zip`（商談用資料.pptx ＋ 店舗POP.pptx を同梱, issue 202607231301） |
| F-09 | 本部担当用ページ（企業G単位 店舗別 推進園数） | 企業G名称、半径(km) | 企業名称列を含む店舗別 推進園数の表・CSV（`{企業G名称}_{半径}km_店舗別推進園数.csv`）。マップなし（issue 202607231301） |
| F-05 | 企業単位データ一括ダウンロード | 企業名称、半径(km) | `{企業名称}_{半径}km.csv`（単一CSV、cp932、企業＋半径で絞込） |
| F-08 | 店舗別 推進園数サマリ | 企業名称、半径(km) | 小売店マスタに圏内推進園数を left join した表（圏内0件は 推進園数=0） |
...
- その下に施設カード（高さ約 `56px` ／件）を縦積み
  ...
  - 番号バッジ右に施設名称（太字 `14px`、`#111827`）と距離（`12px`、`#6B7280`、`約{距離:.1f}km`、小数第1位まで, issue 202607231301）
  - カード間: 下罫線 `1px` `#E5E7EB`
...
企業単位の店舗別 PNG 一括 ZIP（`build_png_zip`）は廃止した（issue 202607221128、§8.5 の
pptx 出力に置換）。合成 PNG（`lib/png_builder.py`）・PNG ZIP（`build_png_zip`）ロジックは
当面残置するが、画面からは参照しない。
一方、`lib/zip_builder.build_pptx_zip` は現役で、商談用資料と店舗POP の pptx を 1 つの ZIP に
まとめて 1 クリックでダウンロードするのに使う（§6.2、issue 202607231301）。

### 8.5 pptx生成（商談用資料 / 店舗POP）

選択中の 1 店舗について、地図画像を PowerPoint テンプレートの画像プレースホルダーへ貼り付けた
pptx（1 スライド）を生成する（`lib/pptx_builder.py`、`python-pptx`）。

- **地図画像:** `lib.static_map.render_static_map(srow, fac, 半径, width=map_width, height=map_height)`
  の地図のみ PNG（合成はしない）。対話地図（`build_map`）と体裁を合わせるため、寸法・表示ズーム
  （viewport は縦=`map_height`）を対話地図と一致させる（issue 202607221245）。
- **テンプレート取得:** `load_template_bytes(kind)` が `[pptx] template_dir/{shoudan_template|pop_template}`
  を Volume（`WorkspaceClient().files.download`）から取得。失敗時は `local_fallback`（`images/template.pptx`）。
- **貼付:** `build_store_pptx(template_bytes, map_png)` が 1 枚目スライドの画像プレースホルダー矩形内へ
  **アスペクト比を保ったまま**（レターボックス・中央配置）`add_picture` で貼付し、元プレースホルダーを
  削除する。`insert_picture` はプレースホルダー比率にクロップして体裁がずれるため使わない
  （issue 202607221245）。
- **依存/実行環境:** `python-pptx` は pure-Python でブラウザ不要のため Databricks Apps 上でも動作する。

## 9. ファイル構成

```
/
├ app.py                  # エントリ。st.navigation で 店舗担当用/本部担当用/データ更新（+テーマ設定は config で出し分け）
├ views/                  # 画面（pages/ は使わない＝自動マルチページ回避）
│  ├ main_page.py         # 店舗担当用ページ（コントロール＋DL＋地図＋施設リスト＋出典）
│  ├ hq_page.py           # 本部担当用ページ（企業G＋半径→店舗別 推進園数の表・CSV、マップなし）
│  ├ upload_page.py       # データ更新（推進園/店舗/RDP の Excel .xlsx/.xls を Volume へ格納、拡張子は保持）
...
│  ├ png_builder.py       # PNG 合成（Pillow、現在は未使用・残置）
│  ├ pptx_builder.py      # 商談用資料/店舗POP の pptx 生成（テンプレDL＋画像プレースホルダー挿入）
│  ├ volume.py            # Unity Catalog Volume 書き込み（アップロード。削除はベース名単位）
│  └ zip_builder.py       # ZIP 生成（画像ZIP＋商談資料・店舗POP の pptx 同梱ZIP, build_pptx_zip）
├ config/
