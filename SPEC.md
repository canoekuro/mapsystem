...
地図（`st_folium`）の直下に、選択中の 1 店舗を対象とした pptx ダウンロードボタンを 2 つ横並びで配置する
（issue 202607221128）。地図画像（`lib.static_map.render_static_map` の地図のみ PNG）を PowerPoint
テンプレートの画像プレースホルダーへ貼り付けた pptx を、`@st.cache_data` で生成して
`st.download_button` から返す（地図 PNG は 1 回だけ生成し両テンプレで使い回す）。

- `st.download_button("商談用資料ダウンロード", ... file_name=f"{小売店名称}_商談用資料.pptx")`（テンプレ `to_shoudan.pptx`）
- `st.download_button("店舗POPダウンロード", ... file_name=f"{小売店名称}_店舗POP.pptx")`（テンプレ `to_pop.pptx`）

テンプレは `[pptx] template_dir` 配下の Volume から取得し、失敗時は同梱の `local_fallback`
（`images/template.pptx`）を用いる（§8.5）。

### 6.2 データダウンロード（企業名称直下の折りたたみ）

企業名称の直下に `st.expander("データダウンロード")`（初期は折りたたみ）を置く。都道府県での絞り込み・
店舗別 PNG 一括画像ダウンロードは廃止した（issue 202607221128）。

- **ローデータダウンロード**: 取得済み企業全体（`企業名称==指定企業 かつ 距離<=半径`）の全行を
  `小売店名称`・`距離` 昇順で 1 つの CSV に出力する。
  - エンコーディング: **cp932**（Excel 互換。`to_csv(index=False).encode("cp932", errors="replace")`）
  - ファイル名: `{企業名称}_{半径}km.csv`、`st.download_button` で直接生成。
...
- **地図画像:** `lib.static_map.render_static_map(srow, fac, 半径)` の地図のみ PNG（合成はしない）。
...
- **挿入:** `build_store_pptx(template_bytes, map_png)` が 1 枚目スライドの画像プレースホルダー
  （`insert_picture`）へ貼付。プレースホルダー非対応テンプレでは、その位置・サイズへ `add_picture` し
  元プレースホルダーを削除するフォールバックで吸収する。
