...
`st.navigation` の別ページ「テーマ設定」。配色を画面から調整する。

- 調整対象: 推進園区分の色（凡例・マーカー）、区分フォールバック色、半径円の線色、
  半径円の塗り透明度（`st.slider` 0.0–0.5）、見出し帯・施設リストヘッダー色、店舗マーカー色。
  色は `st.color_picker` で選ぶ。
- プレビュー: 選択中の配色で見出し帯・凡例・施設リストバッジ・半径円＋店舗を HTML で表示。
- 保存: 「保存」で `config/theme.toml` の `[theme]` を書き換える（`lib/colors.save_theme`）。
...
- 反映範囲: テーマは描画時に解決するため、保存後は地図・ダウンロードPNGの双方へ反映される。
...
| F-07 | テーマ設定 | 各色・透明度 | `config/theme.toml` を更新し地図/PNGへ反映 |
