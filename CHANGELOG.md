...
- change: 周辺マップ概要の施設リストを image2.png の指摘に合わせて微調整（issue 20260717）。
  - 施設名を1行固定にし、はみ出す名称は末尾を `…` で省略（折り返し廃止）。
  - 施設リストの罫線（カード下線・列の区切り線）を削除。
  - 施設リストの帯・本体を「2 列ぶんの固定幅」にし、初期表示は 2 列ぴったり・3 列目以降は
    横スクロールで到達（見切れ防止）。列幅を 230→250px へ拡張。
  - 「施設リスト」帯の上に余白を追加。マップ列と施設リスト列の比率を各固定幅（マップ幅 :
    施設リスト2列幅）に合わせ、両者の間の余白を縮小。`views/main_page.py` のみ変更。
- change: 周辺マップ概要の施設リストを image3.png の指摘に合わせて調整（issue 202607171547）。
  - 「施設リスト」帯・本体を col_list 全幅（`width:100%`）にし、上の「周辺マップ概要」帯と
